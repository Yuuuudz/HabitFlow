...
function initCoinFlip() {
  ...
  let history = JSON.parse(localStorage.getItem('hf_coin_history') || '[]');
  renderCoinHistory();

  btn.addEventListener('click', () => {
    if (coinEl.classList.contains('flipping')) return;
    ...
    setTimeout(() => {
      ...
      const q = document.getElementById('coinQuestion').value.trim();
      const entry = q ? `${q}: ${result}` : result;
      history.unshift(entry);
      if (history.length > 8) history.pop();
      localStorage.setItem('hf_coin_history', JSON.stringify(history));
      renderCoinHistory();
    }, 1050);
  });
  ...
  function renderCoinHistory() {
    historyEl.innerHTML = history.slice(0, 6).map(h =>
      `<span class="coin-history-tag">${h}</span>`
    ).join('');
  }
}
...
function initRoulette() {
  ...
  const habits = [
    { icon: '💧', name: 'Beber 1 copo de água agora' },
    { icon: '📚', name: 'Ler por 10 minutos' },
    { icon: '🏃', name: 'Dar 500 passos' },
    { icon: '🧘', name: 'Respirar fundo por 2 min' },
    { icon: '✍️', name: 'Escrever 3 gratidões' },
    { icon: '💪', name: 'Fazer 10 flexões' },
    { icon: '🌿', name: 'Sair 5 min ao ar livre' },
  ];
  ...
  btn.addEventListener('click', () => {
    ...
    setTimeout(() => {
      const picked = habits[Math.floor(Math.random() * habits.length)];
      inner.textContent = picked.icon;
...
function initWaterTracker() {
  ...
  const MAX = 8;
  const todayKey = 'hf_water_' + new Date().toISOString().slice(0, 10);
  ...
  function render() {
    ...
    mlEl.textContent = `${ml} / 2000ml`;
  }

  function save() { localStorage.setItem(todayKey, String(count)); }

  addBtn.addEventListener('click', () => {
    if (count < MAX) { count++; save(); render(); }
  });
  resetBtn.addEventListener('click', () => { count = 0; save(); render(); });
  render();
}
...
function initGratitude() {
  const saveBtn = document.getElementById('gratitudeSave');
  const streakEl = document.getElementById('gratitudeStreak');
  if (!saveBtn) return;

  const todayKey = 'hf_gratitude_' + new Date().toISOString().slice(0, 10);
  ...
  if (saved) {
    ...
    streakEl.textContent = '✅ Salvo hoje!';
  }
  ...
  saveBtn.addEventListener('click', () => {
    ...
    localStorage.setItem(todayKey, JSON.stringify(vals));
    streakEl.textContent = '✅ Gratidões salvas!';
